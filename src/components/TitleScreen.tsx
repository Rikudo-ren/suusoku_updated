  ...
  return (
    <div className="relative h-full w-full overflow-y-auto overflow-x-hidden overscroll-contain">
      ...
      <div className="relative z-10 flex min-h-full w-full flex-col items-center px-5 pb-8 pt-4 md:justify-center md:py-5">
            ...
            {editingName ? (
              <span className="flex items-center gap-1.5">
                <span className="flex flex-col items-start">
                  ...
                  {nameError && <span className="mt-0.5 font-mono2 text-[9px] tracking-wider text-rose-400">{nameError}</span>}
                </span>
                <button onClick={commitName} disabled={nameSaving} className="clip-chip bg-cyan-400/80 px-2 py-1 font-mono2 text-[9px] font-black text-black disabled:opacity-50">{nameSaving ? "..." : "SAVE"}</button>
              </span>
            ) : (
              <button onClick={() => { sfxUI(); setNameDraft(name); setEditingName(true); }} title="クリックしてパイロット名を変更" className="clip-chip border border-white/15 bg-black/40 px-2.5 py-1 font-ui text-[11px] font-bold tracking-wider text-white/75 hover:border-cyan-300/50 hover:text-cyan-100">PILOT: {name}</button>
            )}
        ...
        <div className="mt-3 mb-2 w-full max-w-3xl px-1 md:mt-4 md:mb-3">
          <div className="mx-auto max-w-xl overflow-hidden rounded-sm border border-cyan-300/15 bg-black/25 p-1">
